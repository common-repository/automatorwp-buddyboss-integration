...
class AutomatorWP_BuddyBoss_Publish_Activity extends AutomatorWP_Integration_Trigger {
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'label'             => __( 'User writes an activity stream message', 'automatorwp-buddyboss' ),
            'select_option'     => __( 'User <strong>writes</strong> an activity stream message', 'automatorwp-buddyboss' ),
            /* translators: %1$s: Number of times. */
            'edit_label'        => sprintf( __( 'User writes an activity stream message %1$s time(s)', 'automatorwp-buddyboss' ), '{times}' ),
            'log_label'         => __( 'User writes an activity stream message', 'automatorwp-buddyboss' ),
    ...
    public function log_fields( $log_fields, $log, $object ) {
        ...
        $log_fields['activity_content'] = array(
            'name' => __( 'Activity Content', 'automatorwp-buddyboss' ),
            'desc' => __( 'The activity content.', 'automatorwp-buddyboss' ),
            'type' => 'text',
        );
